...
class qqsync(Plugin):
    ...
    def set_player_visitor_permissions(self, player):
        ...
        try:
            ...
            chat_permissions = [
                ...
                "minecraft.command.w",
                "minecraft.command.whisper",
                "endstone.command.say",
                "endstone.command.tell",
                "endstone.command.me"
            ]
            ...
            destructive_permissions = [
                ...
                "endstone.command.setblock",
                "endstone.command.fill",
                "endstone.command.give",
                "endstone.command.clear",
                "endstone.command.kill",
                "endstone.command.gamemode",
                "endstone.command.tp"
            ]
            
            # 禁止攻击相关权限
            attack_permissions = [
                "minecraft.interact.entity",
                "minecraft.attack.entity",
                "minecraft.damage.entity",
                "minecraft.hit.entity",
                "minecraft.pvp",
                "minecraft.combat",
                "minecraft.hurt.entity",
                "minecraft.kill.entity",
                "endstone.interact.entity",
                "endstone.attack.entity",
                "endstone.damage.entity",
                "endstone.hit.entity",
                "endstone.pvp",
                "endstone.combat",
                "endstone.hurt.entity",
                "endstone.kill.entity",
                # 添加更多可能的权限名称
                "attack",
                "damage",
                "combat",
            ...
            # 设置禁止权限
            for perm in chat_permissions + destructive_permissions + attack_permissions:
    ...
    def _handle_qq_form_submit(self, player, form_data):
        ...
        try:
            ...
            if not qq_input or not qq_input.isdigit():
                player.send_message(f"{ColorFormat.GRAY}[QQsync] {ColorFormat.RED}请输入有效的QQ号！{ColorFormat.RESET}")
                # 只有在启用强制绑定时才重新显示表单
                if self.get_config("force_bind_qq", True):
                    self.server.scheduler.run_task(
                        self,
    ...
    def _handle_verification_submit(self, player, form_data):
        ...
        try:
            ...
            if player.name not in _pending_verifications:
                player.send_message(f"{ColorFormat.GRAY}[QQsync] {ColorFormat.RED}验证信息已过期，请重新绑定！{ColorFormat.RESET}")
                return
            
            pending_info = _pending_verifications[player.name]
            
            # 检查验证码是否过期（5分钟）
            if time.time() - pending_info["timestamp"] > 300:
                del _pending_verifications[player.name]
                if pending_info["qq"] in _verification_codes:
                    del _verification_codes[pending_info["qq"]]
                player.send_message(f"{ColorFormat.GRAY}[QQsync] {ColorFormat.RED}验证码已过期，请重新绑定！{ColorFormat.RESET}")
                return
    ...
    def _handle_player_left_group(self, player):
        ...
        try:
            ...
            player.send_message(f"{ColorFormat.GRAY}[QQsync] {ColorFormat.RED}已自动切换为访客权限{ColorFormat.RESET}")
            player.send_message(f"{ColorFormat.GRAY}[QQsync] {ColorFormat.AQUA}重新加入QQ群后权限将自动恢复{ColorFormat.RESET}")
            
            # 记录日志
            player_qq = self.get_player_qq(player.name)
            self.logger.info(f"玩家 {player.name} (QQ: {player_qq}) 因退群被设置为访客权限")
            
        except Exception as e:
            self.logger.error(f"处理玩家退群失败: {e}")
